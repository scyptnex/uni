...
%\begin{lemma}
%\label{INVERSEEDGE}
%In the inversion of a feasible solution to a multiterminal cut problem, the endpoints of an edge are assigned to different processors if an only if that edge is in the solution.
%
%\begin{align}
%	\nonumber \forall S \in F : (u, v) \in S \Leftrightarrow \gamma'(S)(u) \neq \gamma'(S)(v) \wedge (u,v) \in E'
%\end{align}
%\end{lemma}
%\begin{proof}
%
